...
/// A node in the tree, combining links to other nodes as well as the
/// actual data.
...
public:
  ...
  //			{
  //			return (std::logic_error::what()+std::string(";
  //")+stacktrace).c_str();
  //			}
  //
...
public:
  ...
  tree<T, tree_node_allocator> &operator=(
      tree<T, tree_node_allocator> &&); // move assignment
  ...
  /// the given iterator. If 'walk_back=true', a depth=0 iterator will be
  /// taken from the beginning of the sibling range, not the current node.
  fixed_depth_iterator begin_fixed(const iterator_base &, unsigned int,
                                   bool walk_back = true) const;
  /// Return fixed-depth end iterator.
  fixed_depth_iterator end_fixed(const iterator_base &,
                                 unsigned int) const;
  ...
  /// Return a path (to be taken from the 'top' node) corresponding to a
  /// node in the tree. The first integer in path_t is the number of steps
  /// you need to go 'right' in the sibling chain (so 0 if we go straight
  /// to the children).
  path_t path_from_iterator(const iterator_base &iter,
                            const iterator_base &top) const;
  /// Return an iterator given a path from the 'top' node.
  iterator iterator_from_path(const path_t &,
                              const iterator_base &top) const;

  /// Return iterator to the parent of a node. Throws a `navigation_error`
  ...
  /// Insert node (with children) pointed to by subtree as previous sibling
  /// of node pointed to by position. Does not change the subtree itself
  /// (use move_in or move_in_below for that).
  template <typename iter>
  ...
  template <typename iter>
  iter reparent(iter position, sibling_iterator begin,
                sibling_iterator end);
  /// Move all child nodes of 'from' to be children of 'position'.
  template <typename iter> iter reparent(iter position, iter from);

  /// Replace node with a new node, making the old node (plus subtree) a
  /// child of the new node.
  template <typename iter> iter wrap(iter position, const T &x);
  /// Replace the range of sibling nodes (plus subtrees), making these
  ...
  /// Move 'source' node (plus its children) to become the previous sibling
  /// of 'target'.
  ...
  /// Extract the subtree starting at the indicated node, removing it from
  /// the original tree.
  tree move_out(iterator);
  /// Inverse of take_out: inserts the given tree as previous sibling of
  /// indicated node by a move operation, that is, the given tree becomes
  ...
  /// As above, but now make the tree the nth child of the indicated node
  /// (if possible).
  ...
  /// As above, but using two trees with a single top node at the 'to' and
  /// 'from' positions.
  ...
  /// Exchange the node (plus subtree) with its sibling node (do nothing if
  /// no sibling present).
  void swap(sibling_iterator it);
  /// Exchange two nodes (plus subtrees). The iterators will remain valid
  /// and keep pointing to the same nodes, which now sit at different
  ...
  /// Compute the depth to the root, counting all levels for which
  /// predicate returns true.
  template <class Predicate>
  static int depth(const iterator_base &, Predicate p);
  /// Compute the depth distance between two nodes, counting all levels for
  /// which predicate returns true.
  template <class Predicate>
  static int distance(const iterator_base &top,
                      const iterator_base &bottom, Predicate p);
  /// Determine the maximal depth of the tree. An empty tree has
  /// max_depth=-1.
  int max_depth() const;
  ...
  /// Count the number of siblings (left and right) of node at iterator.
  /// Total nodes at this level is +1.
  unsigned int number_of_siblings(const iterator_base &) const;
  /// Determine whether node at position is in the subtrees with indicated
  /// top node.
  ...
  /// Find the lowest common ancestor of two nodes, that is, the deepest
  /// node such that both nodes are descendants of it.
  ...
  static sibling_iterator child(const iterator_base &position,
                                unsigned int);
  ...
  /// pointers in the tree (which will also trigger a valgrind error in
...
// template <class T, class tree_node_allocator>
// bool operator<(const typename tree<T, tree_node_allocator>::iterator&
// one, 					const typename tree<T, tree_node_allocator>::iterator& two)
// 	{
...
void tree<T, tree_node_allocator>::head_initialise_()
{
  ...
  std::allocator_traits<decltype(alloc_)>::construct(alloc_, head,
                                                     tree_node_<T>());
  std::allocator_traits<decltype(alloc_)>::construct(alloc_, feet,
                                                     tree_node_<T>());
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::erase_right_siblings(
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::erase_left_siblings(
    const iterator_base &it)
{
  if (it.node == 0)
    return;
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<
    T, tree_node_allocator>::begin() const
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<
    T, tree_node_allocator>::end() const
{
  return pre_order_iterator(feet);
}

template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::breadth_first_queued_iterator tree<
    T, tree_node_allocator>::begin_breadth_first() const
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::breadth_first_queued_iterator tree<
    T, tree_node_allocator>::end_breadth_first() const
{
  return breadth_first_queued_iterator();
}

template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::post_order_iterator tree<
    T, tree_node_allocator>::begin_post() const
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::post_order_iterator tree<
    T, tree_node_allocator>::end_post() const
{
  return post_order_iterator(feet);
}

template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::fixed_depth_iterator tree<
    T, tree_node_allocator>::begin_fixed(const iterator_base &pos,
                                         unsigned int dp,
                                         bool walk_back) const
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::fixed_depth_iterator tree<
    T, tree_node_allocator>::end_fixed(const iterator_base &pos,
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator tree<
    T, tree_node_allocator>::begin(const iterator_base &pos)
{
  assert(pos.node != 0);
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator tree<
    T, tree_node_allocator>::end(const iterator_base &pos)
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator tree<
    T, tree_node_allocator>::begin_leaf() const
{
  tree_node *tmp = head->next_sibling;
  if (tmp != feet)
  {
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator tree<
    T, tree_node_allocator>::end_leaf() const
{
  return leaf_iterator(feet);
}

template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::path_t tree<
    T, tree_node_allocator>::path_from_iterator(const iterator_base &iter,
                                                const iterator_base &top)
    const
{
  path_t path;
  ...
  do
  {
    if (path.size() > 0)
      walk = walk->parent;
    int num = 0;
    while (walk != top.node && walk->prev_sibling != 0 &&
           walk->prev_sibling != head)
    {
      ++num;
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::iterator tree<
    T, tree_node_allocator>::iterator_from_path(const path_t &path,
                                                const iterator_base &top)
    const
{
  iterator it = top;
  ...
  for (size_t step = 0; step < path.size(); ++step)
  {
    ...
    if (walk == 0)
      throw std::range_error(
          "tree::iterator_from_path: no more nodes at step " +
          std::to_string(step));
    ...
    for (int i = 0; i < path[step]; ++i)
    {
      ...
      if (walk == 0)
        throw std::range_error(
            "tree::iterator_from_path: out of siblings at step " +
            std::to_string(step));
    }
  }
  it.node = walk;
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator tree<
    T, tree_node_allocator>::begin_leaf(const iterator_base &top) const
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator tree<
    T, tree_node_allocator>::end_leaf(const iterator_base &top) const
{
...
{
  if (position.node == 0)
    throw navigation_error(
        "tree: attempt to navigate from null iterator.");
...
iter tree<T, tree_node_allocator>::append_child(iter position)
{
  ...
  tree_node *tmp =
      std::allocator_traits<decltype(alloc_)>::allocate(alloc_, 1, 0);
...
iter tree<T, tree_node_allocator>::append_child(iter position, const T &x)
{
  ...
  // top node to an empty tree. From version 1.45 the top element should be
  // added using 'insert'. See the documentation for further information,
  // and sorry about the API change.
  assert(position.node != head);
  assert(position.node != feet);
  assert(position.node);
...
iter tree<T, tree_node_allocator>::append_child(iter position, T &&x)
{
  ...
  std::allocator_traits<decltype(alloc_)>::construct(
      alloc_, tmp); // Here is where the move semantics kick in
  std::swap(tmp->data, x);
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<
    T, tree_node_allocator>::set_head(const T &x)
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<
    T, tree_node_allocator>::set_head(T &&x)
{
  assert(head->next_sibling == feet);
  return insert(iterator(feet), x);
}
...
iter tree<T, tree_node_allocator>::insert(iter position, const T &x)
{
  ...
  {
    if (tmp->parent) // when inserting nodes at the head, there is no
                     // parent
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator tree<
    T, tree_node_allocator>::insert(sibling_iterator position, const T &x)
{
  tree_node *tmp =
      std::allocator_traits<decltype(alloc_)>::allocate(alloc_, 1, 0);
  std::allocator_traits<decltype(alloc_)>::construct(alloc_, tmp, x);
                                           ...
                                           const iterator_base &from)
{
  ...
  std::allocator_traits<decltype(alloc_)>::deallocate(alloc_, current_to,
                                                      1);
  current_to = tmp;
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator tree<
    T, tree_node_allocator>::replace(sibling_iterator orig_begin,
                                     sibling_iterator orig_end,
                                     sibling_iterator new_begin,
                                     sibling_iterator new_end)
{
...
template <class T, class tree_node_allocator>
template <typename iter>
iter tree<T, tree_node_allocator>::wrap(iter from, iter to, const T &x)
{
  assert(from.node != 0);
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator tree<
    T, tree_node_allocator>::move_before(sibling_iterator target,
                                         sibling_iterator source)
{
  tree_node *dst = target.node;
  tree_node *src = source.node;
...
template <class T, class tree_node_allocator>
template <typename iter>
iter tree<T, tree_node_allocator>::move_in(iter loc, tree &other)
{
...
template <class T, class tree_node_allocator>
template <typename iter>
iter tree<T, tree_node_allocator>::move_in_below(iter loc, tree &other)
{
  if (other.head->next_sibling == other.feet)
    return loc; // other tree is empty
...
template <class T, class tree_node_allocator>
template <typename iter>
iter tree<T, tree_node_allocator>::move_in_as_nth_child(iter loc, size_t n,
                                                        tree &other)
{
  if (other.head->next_sibling == other.feet)
    return loc; // other tree is empty
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::merge(iterator to, iterator from,
                                         bool duplicate_leaves)
{
                                        ...
                                        StrictWeakOrdering comp, bool deep)
{
  ...
  if (prev == 0)
  {
    if ((*nit)->parent != 0) // to catch "sorting the head" situations,
  ...
  if (next == 0)
  {
    if ((*eit)->parent != 0) // to catch "sorting the head" situations,
                             // when there is no parent
...
{
  assert(from !=
         to); // if from==to, the range is empty, hence no tree to return.
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::subtree(tree &tmp,
                                           sibling_iterator from,
...
template <class T, class tree_node_allocator>
template <class Predicate>
int tree<T, tree_node_allocator>::distance(const iterator_base &top,
                                           const iterator_base &bottom,
                                           Predicate p)
{
...
{
  int maxd = -1;
  for (tree_node *it = head->next_sibling; it != feet;
       it = it->next_sibling)
...
bool tree<T, tree_node_allocator>::is_in_subtree(
    const iterator_base &it, const iterator_base &top) const
{
...
typename tree<T, tree_node_allocator>::iterator tree<T,
                                                     tree_node_allocator>::
    lowest_common_ancestor(const iterator_base &one,
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator tree<
    T, tree_node_allocator>::sibling(const iterator_base &it,
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator tree<
    T, tree_node_allocator>::child(const iterator_base &it,
                                   unsigned int num)
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator tree<
    T, tree_node_allocator>::iterator_base::begin() const
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator tree<
    T, tree_node_allocator>::iterator_base::end() const
{
  sibling_iterator ret(0);
  ret.parent_ = node;
  return ret;
...
template <class T, class tree_node_allocator>
unsigned int tree<
    T, tree_node_allocator>::iterator_base::number_of_children() const
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator &tree<
    T, tree_node_allocator>::pre_order_iterator::operator++()
{
  assert(this->node != 0);
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<
    T, tree_node_allocator>::pre_order_iterator::operator++(int)
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator &tree<
    T, tree_node_allocator>::pre_order_iterator::next_skip_children()
{
  (*this).skip_children();
  (*this)++;
  return *this;
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator &tree<
    T, tree_node_allocator>::pre_order_iterator::operator+=(unsigned int
                                                                num)
{
  while (num > 0)
  {
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::post_order_iterator &tree<
    T, tree_node_allocator>::post_order_iterator::operator++()
{
  assert(this->node != 0);
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::post_order_iterator tree<
    T, tree_node_allocator>::post_order_iterator::operator++(int)
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::post_order_iterator &tree<
    T, tree_node_allocator>::post_order_iterator::operator+=(unsigned int
                                                                 num)
{
  while (num > 0)
  {
...
template <class T, class tree_node_allocator>
bool tree<T, tree_node_allocator>::breadth_first_queued_iterator::
operator!=(const breadth_first_queued_iterator &other) const
{
  if (other.node != this->node)
    return true;
  else
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::breadth_first_queued_iterator &tree<
    T, tree_node_allocator>::breadth_first_queued_iterator::operator++()
{
  assert(this->node != 0);
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::breadth_first_queued_iterator tree<
    T, tree_node_allocator>::breadth_first_queued_iterator::operator++(int)
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::breadth_first_queued_iterator &tree<
    T, tree_node_allocator>::breadth_first_queued_iterator::
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::fixed_depth_iterator &tree<
    T, tree_node_allocator>::fixed_depth_iterator::operator++()
{
  assert(this->node != 0);
  ...
  upper:
    do
    {
      ...
      {
        this->node = 0; // FIXME: return a proper fixed_depth end iterator
                        // once implemented
        return *this;
      }
      this->node = this->node->parent;
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::fixed_depth_iterator tree<
    T, tree_node_allocator>::fixed_depth_iterator::operator++(int)
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::fixed_depth_iterator &tree<
    T, tree_node_allocator>::fixed_depth_iterator::operator-=(unsigned int
                                                                  num)
{
  while (num > 0)
  {
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator &tree<
    T, tree_node_allocator>::sibling_iterator::operator++()
{
  if (this->node)
    this->node = this->node->next_sibling;
  return *this;
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator tree<
    T, tree_node_allocator>::sibling_iterator::operator++(int)
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator &tree<
    T, tree_node_allocator>::sibling_iterator::operator+=(unsigned int num)
{
  while (num > 0)
  {
    ++(*this);
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::tree_node *tree<
    T, tree_node_allocator>::sibling_iterator::range_first() const
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::tree_node *tree<
    T, tree_node_allocator>::sibling_iterator::range_last() const
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator &tree<
    T, tree_node_allocator>::leaf_iterator::operator++()
{
  assert(this->node != 0);
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator tree<
    T, tree_node_allocator>::leaf_iterator::operator++(int)
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator &tree<
    T, tree_node_allocator>::leaf_iterator::operator+=(unsigned int num)
{
  while (num > 0)
  {
    ++(*this);
